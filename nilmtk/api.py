...
class API:
    ...
    def experiment(self):
        ...
        train_set = self.train_datasets_dict

        for model_name, clf in self.methods.items():
            ...
            if self.chunk_size:
                ...
                if clf.chunk_wise_training:
                    ...
                    for i in range(n_epochs):
                        self.train_chunk_wise(clf, train_set, i)

                # if the model doesn't support chunk wise training
                else:
                    print("Joint training for ", clf.MODEL_NAME)
                    self.train_jointly(clf, train_set)

            # If the user does not wish to perform chunk wise training
            else:
                print("Joint training for ", clf.MODEL_NAME)
                self.train_jointly(clf, train_set)
        ...
        for dataset in d:
            ...
            for building in d[dataset]["buildings"]:
                ...
                for chunk_num, chunk in enumerate(
                    ...
                    clf.partial_fit(
                        self.train_mains, self.train_submeters, current_epoch
                    )
    ...
    def train_jointly(self, clf, datasets):
        ...
        for dataset in datasets:
            ...
            for building in datasets[dataset]["buildings"]:
                print("Loading building...", building)
                train.set_window(
                    start=datasets[dataset]["buildings"][building]["start_time"],
        ...
        if self.site_only != True:
            ...
            for metric in self.metrics:
                ...
                for clf_name, clf in classifiers:
                    computed_metric[clf_name] = self.compute_loss(
                        gt_overall, pred_overall[clf_name], loss_function
                    )
